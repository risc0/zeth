...
use zeth_primitives::{
    ...
    transactions::{
        ...
        linea::{LineaTxEssence, TxEssenceLineaDeposited},
        TxEssence,
    },
...
pub struct LineaTxExecStrategy {}

impl TxExecStrategy<LineaTxEssence> for LineaTxExecStrategy {
    fn execute_transactions<D>(
        mut block_builder: BlockBuilder<D, LineaTxEssence>,
    ) -> Result<BlockBuilder<D, LineaTxEssence>>
    where
        ...
        <D as Database>::Error: Debug,
    {
            ...
            .enumerate()
        {
            ...
            let deposit_nonce = (spec_id >= SpecId::CANYON
                && matches!(tx.essence, LineaTxEssence::LineaDeposited(_)))
            .then(|| {
            ...
            match &tx.essence {
                LineaTxEssence::LineaDeposited(deposit) => {
                    #[cfg(not(target_os = "zkvm"))]
                    {
                ...
                LineaTxEssence::Ethereum(essence) => {
                    fill_eth_tx_env(
                        &mut evm.env_mut().tx,
                        alloy_rlp::encode(&tx),
                        essence,
...
fn fill_deposit_tx_env(tx_env: &mut TxEnv, essence: &TxEssenceLineaDeposited, caller: Address) {
    // CHECK: additional tx fields
